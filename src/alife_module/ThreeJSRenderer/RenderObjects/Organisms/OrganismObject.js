...
import { visionSweep } from '../renderUtils';
...
export const visionRangeVisualizer = ( organism ) => {
    switch (organism.TRAITS.VISION_TYPE) {
        case 'RADIAL':
            ...
            radialSweep.position.set(0 , 0, 1 );
            radialSweep.rotation.set(0,0,0);
            radialSweep.scale.set(1,1,1);

            radialSweep.name = 'visionVisualizer';
            return radialSweep;
    }
}
...
class OrganismObject  {
    ...
    constructor (organism, ...coords) {
        ...
        this.renderable.name = organism.id;
    ...
    update(scene) {
        ...
        //scene.add(visionSweep([this.organism.x , this.organism.y , 0], 'RADIAL', this.organism.visionRange));
        //scene.add( new THREE.Mesh( new THREE.CircleGeometry( this.organism.visionRange, 32 ), new THREE.MeshBasicMaterial( { color: 'aqua', side: THREE.DoubleSide } ) ) );
        this.renderable.position.set(this.organism.x, this.organism.y, 0); 
        this.renderable.add( visionRangeVisualizer( this.organism ) );
